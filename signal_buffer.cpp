...
float absmax(float *vector, int len);
...
void SignalBuffer::setDebugReceiver(std::string address, int port) {
	...
	debugSocket = new UdpSocket(address, port);
	debugBlockIndex = 0;
#endif
}
...
void SignalBuffer::sendDebugBlock(uint32_t blockIndex, uint32_t blockLength) {
	...
	int headerLen = 4 * sizeof(int8_t) + sizeof(int8_t)*channels;

	uint32_t dataBytes = headerLen + (blockLength * channels * sizeof(int8_t));
	int8_t *data = new int8_t[dataBytes / sizeof(int8_t)];

	int bl = blockLength, logBl = 0;
	while (bl >>= 1) { ++logBl; }

	data[0] = (int8_t)debugBlockIndex;
	data[1] = (int8_t)channels;
	data[2] = (int8_t)logBl;
	data[3] = (int8_t)1;
	...
	for (int ci = 0; ci < channels; ci++) {
		...
		data[4 + ci] = (int8_t)(cmax * 0x7f);
		for (uint32_t i = 0; i < blockLength; i++) {
			float f = (cmax == 0.0f) ? 0.0f : (getPtrTQ(ci, blockIndex)[i] / cmax);
			if (f > 1.0f) f = 1.0f;
			if (f < -1.0f) f = -1.0f;
			data[headerLen / sizeof(int8_t) + ci * blockLength + i] = (int8_t)(f * 0x7f);
		}
	}
	...
	// this will wrap arround, -127->128
	debugBlockIndex++;
}
